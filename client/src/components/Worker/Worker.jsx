...
export const Worker = ({
  ...
  useEffect(() => {
    ...
    if (Object.keys(user).length !== 0) {
      ...
      if (type === "worker") {
        ...
        nuevoObjeto.Jobs = user.Worker?.Jobs[0].name
          ? user.Worker?.Jobs.map((e) => e.name)
          : user.Worker?.Jobs;
        ...
        if (e[variableComent]) {
          const elemento = {
            ...
            name: e.User
              ...
              : e.Worker.User.lastName + " " + e.Worker.User.name,
  ...
  const ordenarFiltrados = (tipo) => {
    if (tipo == "r") {
      const auxiliar = listaValoraciones.sort((a, b) =>
        a.date > b.date ? 1 : -1
      );
  ...
  const ocultarFilters = (buleano) => {
    if (!buleano) setDisplaying("");
    else setDisplaying("none");
  };
          ...
          <div className="worker">
            ...
            <div className="w-content">
              <div className="w-left">
                {worker.User && worker.User.name ? (
                  <Profile
                    id={user.Worker ? user.Worker.ID : false}
                    ocultarFilters={ocultarFilters}
                    img={worker.User.img}
                    name={worker.User.name}
                    jobs={worker.Jobs}
                    description={info?.Worker?.description}
              ...
              <div className="w-right" style={{ display: displaying }}>
                {finishedJobs > 0 ? (
                  <Stats
                    ...
                    texto={worker.Jobs ? "terminados" : "requeridos"}
                  />
                ) : (
                  <Stats
                    finishedJobs={0}
                ...
                {worker.User ? (
                  <>
                    ...
                    <Opinion
                      ...
                      contratos={valoraciones}
                      tipo={worker.Jobs}
                    />

                    {maxPag > 0 && finishedJobs > 0 ? (
