...
import { getUserDetail, getUserId ,changeStatus} from '../../redux/actions/actions'
...
const Profile = () => {
  ...
  useEffect(()=>{
    if(Object.keys(user).length !== 0){
      ...
      if(popUpsAux.length > 0)
        popUpsAux = popUpsAux.reverse()//Deberia ordenarlos por fecha, para no tener que usar esto que da vuelta cuando quiere las notificaciones jaja
      setPopUps(popUpsAux)
  ...
  useEffect(()=>{
    socket?.on("obtenerNotificacion",({id,img,nombre_emisor,tipo}) => {
      ...
      let popAuxiliar = {}
      popAuxiliar = {type:tipo,viewed:false,img:img,name:nombre_emisor}
      popsAuxiliar.push(popAuxiliar)
      popsAuxiliar = popsAuxiliar.reverse()
      setPopUps(popsAuxiliar)
      let cantidadAuxiliar = 0
      popsAuxiliar?.forEach(e => e.viewed?  cantidadAuxiliar: cantidadAuxiliar++  )
      setcantNotificaciones(cantidadAuxiliar)
    })
  },[socket,popUps])
  ...
  const showPopUps = () => {
    ...
    setPopUps(popUps.map(e => {return {...e,viewed:true}}))//Me pinto freestylear jaja
  ...
  return (
    ...
    <ClickAwayListener onClickAway={DshowPopUps}>
      <div>
      <Badge onClick={showPopUps} badgeContent={cantNotificaciones} color="primary">
